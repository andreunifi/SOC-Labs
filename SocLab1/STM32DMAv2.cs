...
{
    public sealed class STM32DMAv2 : IDoubleWordPeripheral, IKnownSize, IGPIOReceiver, INumberedGPIOOutput
    {
        public STM32DMAv2(Machine machine)
        {
        ...
        {
            public Stream(STM32DMAv2 parent, int streamNo)
            {
                this.parent = parent;
            ...
            private int CalculateSize(int numberOfData, TransferType source_transfer_type) {
                if (source_transfer_type == TransferType.Byte) {
                    // THIS IS FOR BYTE
                    return numberOfData;
                }
                else if (source_transfer_type == TransferType.Word) {
                    // THIS IS FOR HALF WORD
                    return 2 * numberOfData;
                }
                else if (source_transfer_type == TransferType.DoubleWord) {
                    // THIS IS FOR WORD
                    return 4*numberOfData;
                }
                return 0;
            }
            ...
            private Request CreateRequest(int? size = null, int? destinationOffset = null)
            {
                ...
                return new Request(sourceAddress, (uint)(destinationAddress + (destinationOffset ?? 0)), size ?? CalculateSize(numberOfData, sourceTransferType), sourceTransferType, destinationTransferType,
            ...
            private readonly STM32DMAv2 parent;
