...
int cs43l22_play(int16_t *pbuffer, uint32_t size)
{
	...
	if (cs43l22_state != PLAY)
	{
		...
		// powered up, page 38
		i2c_data = 0b10011110;
		status = HAL_I2C_Mem_Write(&CS43L22_I2C, CS43L22_I2C_ADDRESS, CS43l22_POWCON1_REG,
												1, &i2c_data, 1, 100);
		if (status)
		{
			return status;
		...
		cs43l22_state = PLAY;
		status = HAL_I2S_Transmit_DMA(&CS43L22_I2S, (uint16_t * )pbuffer, size);
        if (status)
		{
			return status;
		}
